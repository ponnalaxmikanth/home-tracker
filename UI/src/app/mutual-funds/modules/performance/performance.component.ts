...
export class PerformanceComponent implements OnInit {
  ...
  constructor(private _fundsServiceService: MutualFundsService, private SpinnerService: NgxSpinnerService, private messageService: MessageService) {
    ...
    this.cols = [
      { field: 'rownum', header: '#', type: '#', class: 'row-number', filter: false },
  ...
  exportExcel() {
    import("xlsx").then(xlsx => {
        const worksheet = xlsx.utils.json_to_sheet(this.performance);
        const workbook = { Sheets: { 'data': worksheet }, SheetNames: ['data'] };
        const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
        this.saveAsExcelFile(excelBuffer, "funds_performance");
    });
  }
